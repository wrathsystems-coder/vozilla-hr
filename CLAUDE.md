...
### 2026-05-14 — Sprint 7 — Code-side close (feature/polish complete)
...
**Continuation session (2026-05-14 PM)** — Shipped preostali 6 Sprint 7 code-side itema preko 6 commitova: (1) `feat(cron)` customer feedback day 3/14/30 cron — pure `pickFeedbackDay()` s catch-up semantics (40-day-old lead → day30, ne backfill day3), 3 React Email templates s tracker-URL CTA-ima, `LeadRequests.customer_feedback_emails` group (disabled + dayN*sent_at) preko migracije `20260514_163301`, `/api/cron/customer-feedback` na Vercel daily 04:15 UTC, 14 novih unit testova; (2) `feat(legal)` PDF download — print-friendly approach umjesto pdf-lib dependency: `lib/legal/pages.ts` registry kao single source-of-truth, `(print)/print/[slug]` route group s vlastitim layout-om (no public chrome), @media print A4 CSS s inline href hints, `AutoPrint` client component sa 250ms render-settle delay, robots disallow `/print`; (3) `feat(security)` CSP middleware — Edge runtime nonce gen via Web Crypto, propagiran kroz `x-nonce` request header, dvije politike: public `'self' 'nonce-…' 'strict-dynamic'` + recaptcha/cookiebot allowlist, admin `'unsafe-eval' 'unsafe-inline'` za Lexical; `JsonLd` server component reads `headers()` i emits `<script nonce={…}>`; (4) `feat(sentry)` no-op-when-DSN-empty SDK install — `@sentry/nextjs` 10 + `sentry.{client,server,edge}.config.ts` + `instrumentation.ts` koji forwards na runtime-specific config; `next.config.ts` wraps Payload output u `withSentryConfig` samo kad SENTRY_AUTH_TOKEN set; (5) `test(a11y)` `pnpm test:a11y` script + Playwright axe spec across 17 page templates, fails na serious/critical, soft-warns na moderate/minor, `color-contrast` rule disabled dok brand HEX ostaje `XXX_BRAND*\*`; (6) `docs(sprint7)` `content-editing.md`postao ~95% functional vodič,`pre-launch-checklist.md`updated s checked items + new verification steps,`scripts/generate-docs.ts`proširen s`generateApiRoutesDoc()`koji walka`app/\*\*/route.ts`i emit-a`docs/api-routes.md`(17 endpointa, grouped by prefix). Final verify:`pnpm test`299/299 ✓,`pnpm type-check`✓,`pnpm lint`✓, full integration tests pass nakon`pnpm payload migrate` lokalno (3 nove migracije applied). Placeholder count 247 (+1 vs in-progress entry, dokumentiran u accessibility.md fallback section).
...
### 2026-05-18 — Sprint 8 setup — Catalog bootstrap pipeline + filter foundation + partner rename

- Shipped 6 commita ove sesije pripremajući Sprint 8 (catalog ingest + filter UX): (1) `docs(rules)` — CLAUDE.md dobio pravilo #11 "Filter architecture (load-bearing)": multi-select gdje god ima smisla, OR-within / AND-between, URL spec (comma-separated values per param + range `_od/_do` parovi, parser `URLSearchParams.getAll()` + split-on-comma), numeric filteri = range slider s preset bucket presjecima, faceted counts po dim (jedan count query per facet sa svim filterima APLICIRANIM osim taj jedan), 0-result kombinacije disabled state ne crash, DB layout (typed columns na `model_versions` za sve filterable numeric + `text[]` + GIN za multi-valued enum-e + composite indexi za česte combo filtere + materialised view samo na evidence), SEO canonical (filter combos noindex+follow, brand-only `/marke/{brand}/` ostaje indexabilna), `model_versions` = single source of truth za new-car catalog filtere. (2) `feat(partner)` route rename — git-mv cijelo `/dileri/*` stablo na `/partneri/*` (17 fileova: login, dashboard, lead/[id], profil, reset/[token], zaboravljena-lozinka), middleware 301 redirect (`pathname === "/dileri" || startsWith("/dileri/")` → `/partneri` + preserve search params), middleware matcher fix `.+` → `.*` (homepage `/` sad matcha — pre-existing bug otkriven u smoke testu); `requireDealer()` + Payload session i dalje gate-aju collection `dealers` (interna logika netaknuta). (3) `feat(partner)` HR text rename — sed-pass kroz UI/email/SEO/breadcrumbs/Payload labels: `[Dd]iler` family → `[Pp]artner` family preserving Croatian morphology (diler/dilera/dileru/dilerom/dileri/dilerima/dilerske/dilersko ↔ partner/partnera/partneru/partnerom/partneri/partnerima/partnerske/partnersko); Croatian word-boundary sed je naturally safe — `dealer` (engleski) ne matcha. Mid-pass restored middleware redirect koji je sed accidentally rewrite-ao u `/partneri` → `/partneri` infinite loop. (4) `feat(filter)` design doc + schema — `docs/filter-architecture.md` (autoritativni Sprint 8 dizajn: filter dimensions, URL spec, DB schema extensions, facet count strategy, disabled UX, SEO canonical, implementation plan), `ModelVersions` collection dobiva 10 typed columns (max_speed_kmh, acceleration_0_100_s, boot_capacity_l, weight_kg, length_mm, width_mm, height_mm, wheelbase_mm, doors_count, seats_count) + drivetrain enum (fwd/rwd/awd/4x4) + equipment hasMany (16 options) + colors_available hasMany (11 options); migracija `20260514_211252_sprint8_model_versions_filter_columns` (CREATE TYPE 2 enuma + 2 hasMany join tables + ALTER TABLE ADD COLUMN za 10 numeric + drivetrain enum + FK + 12 filter-performance B-tree indexa: price_eur, year, max_speed_kmh, acceleration_0_100_s, boot_capacity_l, weight_kg + composite (brand_id, body_type_id) + (fuel_type, transmission), itd.); hand-cleaned migration nakon spurious re-adds iz Payload auto-generatora (`oglas` + customer_feedback_emails duplikati uklonjeni). (5) `feat(branding)` deterministic placeholders + OG generator — `lib/branding/placeholder.ts` (FNV-1a 32-bit hash → 12-hue HSL bucket sa fixed sat/light za WCAG kontrast, `brandPlaceholderColor(slug)` + `brandAbbreviation(name)` initials-or-first-2-3-chars + `buildPlaceholderOgImage({headline, caption, colorSlug?})` SVG data-URI generator), `components/catalog/BrandCard.tsx` refactored na colored top half + abbreviation + info bar bottom (zamijenio plain text wordmark fallback); 13 unit testova (determinizam, hue bucket coverage, contrast invariant, abbreviation pravila, SVG XML escape, color-slug independence). Per CLAUDE.md pravilo #1 — agent NE generira brand assete, ali deterministic HSL placeholder čitati kao "designed placeholder", ne "we forgot to upload". (6) `feat(import)` generic CSV vehicle importer — `pnpm import:vehicles --file=<csv> [--mapping=<ts>] [--dry-run] [--limit=N]`: types kontrakt (VehicleMapping + ImportSummary), helpers (EU-decimal parseNumber 12.345,67 → 12345.67, splitMulti semicolon/pipe/comma, toSlug NFD-strip, value-map lookup case-insensitive, parseBoolean da/ne/yes/no), 5-pass orchestrator (parse → map → UPSERT brands by slug → auto-create body_types → UPSERT models by (brand,slug) → UPSERT versions by (model,name)) sa explicit step logging po seed-script konvenciji (no silent try/catch, no subprocess), default English-column mapping s Croatian-aware value maps (Petrol→benzin, Diesel→dizel, Hybrid→hibrid, PHEV→phev, Electric→ev, AWD→awd, Sedan→limuzina, Karavan→karavan, Panorama→panorama, ...), `seeds/template-import-vehicles.csv` (8 redaka × 35 stupaca: Brand, Model, Trim, Body Type, Segment, Generation, Year From/To, Price (EUR), Country, Fuel Types, Engine Type, Engine Displacement, Power (kW/HP), Transmission, Drivetrain, Fuel Consumption, CO2, Max Speed, 0-100, Boot Capacity, Weight, L/W/H/Wheelbase, Doors, Seats, Equipment;-delimited, Colors;-delimited, Active flags) — Audi A4×2, BMW X3×2, Škoda Octavia, VW Golf, Kia EV6, Tesla Model 3; `.gitignore` adds `/data/` da vlasnik može drop-ati real catalog files lokalno bez commit-a (potencijalno 20k+ redaka + unpublished pricing).
- Verify end-to-end: importer dry-run protiv template — 8/8 mapped 0 skipped 0 errors. Real run — +4 brands (Škoda, VW, Kia, Tesla, jer Audi+BMW pre-seed iz Sprinta 1) +4 models +8 versions 0 errors. Idempotency check (second run identical CSV) — 0 created, 6 brands reused + 6 models updated + 8 versions updated. DB spot-check: typed columns (power_hp, max_speed_kmh, boot_capacity_l, weight_kg, drivetrain, price_eur) svi landed correctly; Tesla Model 3 Long Range AWD ima 11 equipment + 6 colors u GIN-indexed join tablicama (`model_versions_equipment` + `model_versions_colors_available`). `pnpm type-check` ✓, `pnpm lint` ✓, `pnpm test tests/unit` 238/238 ✓ (+13 brand-placeholder, +unit gains od prior progress).
- Otvorena za Sprint 8 implementation: (a) UI filter komponente — multi-select chips (Marka/Gorivo/Mjenjač/Pogon/Oprema/Boje/Sjedala) + dvostruko-ručka range slideri (Cijena/Godina/KM/Snaga/Brzina/Prtljažnik/Težina/Ubrzanje) + URL state hook (`useFilterState` koji parsira `URLSearchParams` na arrays + emit-a comma-joined back), shareable + bookmarkable; (b) faceted count fetcher — `lib/catalog/facets.ts` koji za svaki facet dim runa filtered query bez tog facet-a → returns `{ [facet]: { [value]: count } }`, cached `unstable_cache` taggan `model_versions`; (c) listing page za `/nova-vozila/?marka=audi,bmw&gorivo=diesel,hybrid&cijena_od=20000&cijena_do=40000` (filter sidebar + faceted counts + disabled state na 0-result + sortir presjeci + paginacija); (d) brand logo paths/svg upload flow (real wordmarks postupno zamjenjuju deterministic placeholder, koji ostaje fallback); (e) used cars filter extension (parallel `/rabljena-vozila/` ima zasebni filter set — overlap s nova-vozila gdje god deli model_versions JOIN). Sve čeka stvarni CSV od vlasnika — template potvrđuje pipeline radi, ali column names / value maps će biti adjusted u custom mapping-u (`scripts/import/mappings/vlasnik-katalog-2026.ts`) kad CSV stigne.
- Ključne odluke: ovaj sprint je infrastructure-only — design doc + schema + importer + placeholder lib + rename — bez user-facing UI promjena (osim partner labela). Filter UI implementacija čeka stvarni dataset jer column variance + value vocabulary će drive concrete component design (npr. ako CSV ima boje "Pearl White" i "Cosmic Black", value map mora cover-ati prije UI swatch render-a). Importer dizajn pluggable: jedna mapping per CSV vendor (default.ts = English, vlasnik kopira + adjusts). `revalidateTag` warnings tijekom import-a su benign (wrapper već swallows van HTTP scope-a) — ne treba silenced jer prikazuje da hookovi fire (regression catch ako se ikad ulink-aju). Multi-select bezgranično skalabilno za 20k+ vozila jer schema je: typed columns (B-tree friendly range) + GIN indexes (array contains O(log n)) + composite indexi za top combo queries — facet count je N+1 querya gdje N = broj facet dimenzija, svaki s 1h `unstable_cache` TTL tag-an na `model_versions`. Materialised view defer until benchmark pokaže problem (Postgres count(\*) na 20k rows je <100ms bez optimizacije). Brand placeholder per dist HSL ostaje kao fallback PUNO dulje od initial launch-a — partner-uploaded logos će postupno fill-ati popular brands, dok long-tail (npr. SsangYong, Lada) može sjediti na HSL placeholderu indefinitely bez UX štete.

### 2026-05-18 — Sprint 8 follow-up — Importer fix + extended filter dims iz vlasnik CSV-a

- Shipped 4 commita: (1) `fix(import)` — dry-run skipa Payload init + explicit `process.exit(code)` na success path; otkrio sam 5 zombie node procesa koji su visili indefinitely nakon prošlih test runova, jer `getPayload()` drži pg pool + workers koji ne puštaju event loop. Dry-run sad završi za 1s (prije: nikad), real run 4s clean exit, 0 leftover procesa. Plus `chore(import)` cleanup dead-stub. (2) `feat(filter)` — vlasnik dropao 5-vehicle catalog CSV (`data/imports/vehicles-master.csv` u .gitignored mapi) s 14 spec stupaca preko core 35 template-a. Audit + extension napravljen **bez ijedng novog filter mehanizma** — svaki novi dim slot-a u 3 postojeća obrasca (numeric typed column → range slider; single enum → multi-select chip; text[] join + GIN → multi-select). Migracija `20260518_144240_sprint8_extended_filter_dimensions` dodaje 13 novih kolona na `model_versions` (torque_nm, ev_range_km partial-idx, load_capacity_kg, climate_zones, infotainment_screen_in, usb_ports, euro_ncap_stars, airbags_count, engine_config enum, engine_config_notes, eco_norm enum, seat_material_notes, steering_material_notes), 2 nove hasMany join tablice (seat_materials, steering_materials) s GIN-friendly FK + idx pattern, 20 novih equipment enum vrijednosti (aeb, massage_seats, memory_seats, alcantara_seats, vegan_leather_seats, rear_camera, auto_parking, led_lights, night_vision, travel_assist, sport_chrono, pasm, harman_kardon, burmester_audio, bose_audio, touchscreen, voice_assistant, physical_buttons, rotary_controller, touch_sliders) — svaki ALTER TYPE ADD VALUE u zasebnom `db.execute()` (Postgres tx requirement). 10 novih B-tree indexa za filter performance + partial idx na ev_range_km (most rows null za non-EV). Importer extensions: `helpers.ts` dobiva `extractCanonicalTokens(input, patternMap)` koji koristi longest-match-wins + consume-span semantiku (sprječava da "Veganska koža" tagira plain `leather` paralelno), vraća tokene u **input-position order** (leftmost wins) tako da engine_config[0] = primary ICE config ("Inline-4 Atmo + elektromotor" → inline_4, ne electric_motor); `parseInchesFromText()` extracta največu inch vrijednost iz '12" + 5.3"' (headline filter wants main screen, ne cluster); `parseNcapStars()` parsa "5 zvjezdica (2020)" → 5, "Nije testirano" → undefined, clampa out-of-range (0/6+). Default mapping extenzije: FUEL_VALUE_MAP dobio HEV/MHEV/BEV aliase, TRANSMISSION dobio PDK/DSG, DRIVETRAIN dobio 4MATIC/xDrive/Quattro, EQUIPMENT +25 entries, novi ECO_NORM_VALUE_MAP + ENGINE_CONFIG_PATTERN_MAP + SEAT_MATERIAL_PATTERN_MAP (s "tkanin" prefix za Hrvatsku morfologiju koja covera tkanina/tkanine/tkaninom) + STEERING_MATERIAL_PATTERN_MAP; Interface Type free-text fold-a u **postojeći equipment[]** preko regex parse-a (touchscreen/voice_assistant/physical_buttons/rotary_controller/touch_sliders) — keeps sustav unified, bez nove filter dim. Novi CLI tools: `pnpm import:audit-csv <csv>` (dump CSV column coverage + distinct value buckets), `pnpm import:audit-mapping <csv> [N]` (run mapping kroz prvih N rowa + dump payload za diff review). Seller Location intentionally NOT consumed by catalog mapping — pripada `used_car_listings` per-listing s `location_id` FK preko `counties` (postoji od Sprint 4). (3) `docs(progress)` CLAUDE.md Sprint 8 entry update. (4) `feat(filter)` self-contained — sve gore (migration + types + helpers + mapping + run-import + collection + docs + 22 nova unit testa) shipped kao jedan atomic commit.
- Verify end-to-end protiv vlasnik CSV-a: dry-run audit (`audit-mapping.mjs`) — sve 5 vozila imaju ispravno extracted typed columns (Toyota Yaris: inline_4 + fabric + plastic+fabric, BMW X5: inline_6 + leather + leather+alcantara, Mercedes C220d: inline_4 + synthetic_leather+microfiber + leather, Porsche 911: boxer_6 + leather+alcantara + alcantara — euro_ncap_stars=undefined for "Nije testirano", VW ID.4: electric_motor + vegan_leather + leather, eco_norm=bev, ev_range=522km). Real import 0 errors, 5 versions updated. DB join tables: each version ima 11-21 equipment items, 1-2 seat materials, 1-2 steering materials, 7-8 colors. Filter smoke queries pass: numeric range (BTREE: `torque > 400 AND airbags >= 7` returns 2), GIN multi-AND (`leather_seats AND adaptive_cruise` returns 7), GIN multi-OR (`vegan_leather OR alcantara seat materials` returns 2), B-tree enum (`eco_norm = bev` returns VW ID.4). `pnpm type-check` ✓, `pnpm lint` ✓, `pnpm test tests/unit` 260/260 ✓ (+22 import-helpers: 7 extractCanonicalTokens + 6 parseInchesFromText + 5 parseNcapStars + 4 parseNumber regression). Full pre-push suite 334/334 ✓.
- Otvorena za Sprint 8 implementation (nepromijenjeno od prethodnog entry-a): UI filter komponente + faceted count fetcher + listing page sa svim 20+ filter dim. Sad architecture-wise dovršeno za sve atribute koje vlasnik može slati u CSV — implementation faza traži samo komponente. Plus: Seller Location pipeline za used-car import (drugi mapping čita ga + radi postcode→county lookup) — defer until vlasnik šalje used-car CSV.
- Ključne odluke: ekstenzija je striktno additivna — 0 promjena u arhitekturi, samo više kolona. Substring-match pattern logic (longest-wins + consume-span + input-position order) je sad standardna tokenizer semantika koja se može reuse-ati za bilo koji budući free-text → canonical token mapping. Croatian morphology workaround = prefix-pattern (e.g. "tkanin" hvata sve padežne forme); skalira na cijeli HR vokabular dok god mapping author dodaje root patterns umjesto pun-padežnih (koža → "kož", drvo → "drv", etc.). Engine config single-enum + raw notes pattern poboljšava UX: filter koristi canonical (`inline_4` = svih cars s 4-cyl inline), display koristi raw ("Inline-4 Atmo + elektromotor"); nema gubitka informacije. Interface Type fold-an u equipment[] umjesto nove dim — granularni features (touchscreen/voice/buttons) su semantički "vozilo ima X" iste kao panorama/HUD; korisnik filtrira "auto s touchscreenom" istom kontrolom kao "auto s panoramom". Migration koristi 21x `await db.execute(ADD VALUE)` u zasebnim tx-ovima — Postgres requirement, dokumentirano fragile-but-working pattern iz CLAUDE.md (also: oglas migration iz Sprint 7). Audit-mapping CLI tool ostaje permanent dio repo-a (`scripts/audit-import-*.mjs` + npm scripts) — vlasnik može pokreniti pred svaki novi CSV import bez vlasnika imati DB pristup ili pokrenuti pravi import.
